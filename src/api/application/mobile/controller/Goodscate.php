...
class Goodscate extends \think\Controller{
	...
	public function index()
	{
		...
		$list=db('cate')->alias('c')
			...
			->field('c.id, c.cate_name, c.cate_thumb, g.id, g.goods_title, g.goods_img, g.goods_price')
			->where($where)
			->order('c.id desc')
			->limit('0,10')
			->select();
		...
		foreach ($list as $key => $value) {
			 ...
			 $rtn_list[$value['id']]['goods_list'][$value['id']]['goods_title'] =$value['goods_title']; 
			 $rtn_list[$value['id']]['goods_list'][$value['id']]['goods_img'] =$value['goods_img']; 
