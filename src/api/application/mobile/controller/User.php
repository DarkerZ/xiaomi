<?php 
	...
	class User extends \think\Controller{
		...
		public function read(){
			$username = input('username');
			$password = input('password');
			$user = db('user')->where("username",$username)->where("password",md5($password))->find();
			if(empty($user)){
				$user['msg'] = 0;
				return jsonp($user);
			}else{
		...
		public function updateInfo(){
			...
			if(input('type')=='username'){
				...
				$password = input('password');
				$curPsw = input('curPsw');
				$is_right = db('user')->where("id",$id)->where("password",md5($curPsw))->find();
				if(empty($is_right)){
					return 0;
					exit();
				}
				$save_data=array("password"=>md5($password));
